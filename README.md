...
- Reverse vertical scrolling for any non-continuous, line-based mouse-wheel event while leaving trackpad scrolling unchanged. Only the VXE Mouse 1K Dongle is physically verified and supported for `0.1.1`; the implementation does not filter by vendor.
...
Status: `0.1.1` release candidate. The public tag and release are created only after the local TidyTap Release skill completes.
...
The `v0.1.1` release is prepared from a version-matched main tag using the local TidyTap Release skill. For development, build the app locally, then open the resulting app:
