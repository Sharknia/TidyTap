...
Caps Lock 전용 경로는 권한 요청 UI를 만들지 않는다. 휠 경로는 접근성 및 입력 모니터링 권한을 모두 검사하고, 측면 버튼 전용 경로는 접근성 권한만 검사한다. 권한이 없거나 회수되면 event tap은 원본 이벤트를 통과시키고, 해당 토글은 켜진 것처럼 저장/표시하지 않는다. 앱은 사용자가 직접 시스템 설정을 열도록 안내한다.
