...
작성일: 2026-09-07  
상태: 구현 진행 중 (2026-09-07 사용자 승인)  
구현 브랜치: `fix/first-run-permissions-capslock` (`origin/dev`의 `b414f80`에서 분기 후 `v0.1.1` 병합)

이 문서는 앞서 작성한 계획서를 삭제한 뒤, 추가 조사 결과로 새로 작성했다. 제품 코드와 사용자 권한·키 매핑은 변경하지 않았다.
...
- [ ] 입력 모니터링을 별도 승인한 것처럼 표시하지 않고 수동 추가를 요구하지 않는다.
- [ ] 손쉬운 사용만 승인한 최초 설치 환경에서 관련 마우스 기능을 실제 검증한다.
- [ ] `(null)` 환경에서 Caps Lock 활성화·전환·비활성화·복원이 성공한다.
- [ ] 기존 키 매핑을 보존하고 실제 실패 이유를 표시한다.
- [ ] 관련 자동 테스트, 설치·업데이트 검증과 문서 갱신을 완료한다.
