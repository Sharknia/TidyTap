...
확장 시나리오와 실제 macOS 15.1 실행의 미검증 범위는 릴리스 문서에 명시한다. 설치본 교체,
...
사용자의 Finder 기능 실사용 확인 후 릴리스를 승인받았다. 최종 엔진 테스트 101개,
앱 테스트 104개, Release/Preview DMG 워크플로와 릴리스 순서 검사, launch-smoke가 통과했다.
이번 launch-smoke는 사용자 시스템 상태 미변경 해시 검사까지 통과했다. 앞서 기록한
개발 중 스모크 실패와 구분하며, 실제 macOS 15.1 및 확장 하드웨어 검사는 미검증으로 유지한다.
