...
### 0.1.1 최초 설치 후 표시 수정 (2026-09-07)

손쉬운 사용 권한은 이벤트 전송과 수신을 허용한다. `CGPreflightListenEventAccess()`는 수신 가능 여부이며 시스템 설정의 입력 모니터링 항목을 별도로 승인했다는 증거가 아니다. 마우스 기능의 사용자 권한 안내는 손쉬운 사용으로 통일하고 별도 입력 모니터링 승인 행·설정 버튼을 제거한다. 내부 수신 가능 검사와 실제 이벤트 탭 생성·회수 검사는 유지한다.

기존 IPC의 `inputMonitoring` 필드는 수신 가능 결과로 해석한다. 저장 형식 호환성을 위해 유지하며 독립 TCC 승인으로 표시하지 않는다. 기존 앱과 같은 팀의 Developer ID 서명 및 앱/Worker 식별 조건을 유지한다. 새 인증서로 기존 앱의 요구 조건을 충족한 서명 시험은 통과했지만, 실제 업데이트 후 권한 유지와 깨끗한 계정의 최초 승인은 별도 검증 대상이다.

근거: [Apple DTS](https://developer.apple.com/forums/thread/828052). 아래 과거 검증에서 두 항목이 허용으로 표시됐다는 기록은 각 항목의 독립 승인을 증명하지 않는다.

### 중첩 Helper 권한 주체 분리 (기존 조사)
