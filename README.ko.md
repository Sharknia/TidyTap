...
상태: `0.1.1` 릴리스 후보. 공개 태그와 릴리스는 로컬 TidyTap Release 스킬이 완료된 뒤에만 생성합니다.
...
`v0.1.1` 릴리스는 버전이 일치하는 main 태그에서 로컬 TidyTap Release 스킬로 준비합니다. 개발 중에는 앱을 로컬에서 빌드한 뒤 결과물을 여세요.
